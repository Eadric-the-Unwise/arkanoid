...
size_t array_size = sizeof(collision_map);   // calculate size of the array
unsigned char collision_map_ram[array_size]; // ROM to WRAM copy of collision_map
...
void copy_collision_data() // copy ROM to WRAM
{
    ...
    memcpy(ram_pointer, collision_map, array_size);
    // unsigned char COLLISION_MAP_SIZE = sizeof(collision_map);

    // unsigned char *rom_pointer = collision_map;
