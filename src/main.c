...
// UINT8 COLLISION_MAP_SIZE = COLLISION_MAP_SIZE;
...
void copy_collision_map(const unsigned char *rom_map)  // copy ROM to WRAM
{
    unsigned char *rom_pointer = collision_map_ram;  // point to the ROM collision map data
    // Copy data from ROM to RAM
    memcpy(rom_pointer, rom_map, sizeof(collision_map));
}
